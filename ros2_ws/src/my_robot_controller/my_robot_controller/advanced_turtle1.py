...
class Zigzag(py_trees.behaviour.Behaviour): # class for moving turtlsimnode left and right
    def __init__(self, name, node):
        ...
        self.node = node
        self.publisher = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)
        self.timer = self.create_timer(2)  # timer interval for zigzag motion
        self.turn_left = True  # Flag to alternate between left & right turns
    ...
    def update(self):
        twist_msg = Twist()
        twist_msg.linear.x = 0.5  # linear velocity for a zigzag pattern
        if self.turn_left:
        ...
        self.publisher.publish(twist_msg)
        self.turn_left = not self.turn_left  # Toggling turn direction
...
#adding child nodes
    ...
    tree.setup(timeout=30)  # Increasing the timeout for the zigzag motion
