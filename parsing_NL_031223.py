...
xml_file_path = r"D:\Master HCI\2. Semester\Projekt\behavior_tree_dataset\behaviortreecpp\Car_BT.xml"
    ...
    def describe(element, indent=""):
        ...
        if element.tag.endswith("Action") and "ID" in element.attrib:
            if parent_condition:
                ...
                description += f"{indent} '{element.attrib['ID']}'\n"
        elif element.tag.endswith("Sequence"):
            # Check the number of actions in the sequence
            num_actions = sum(1 for _ in element.iter("Action"))
            if num_actions >= 2:
                description += f"{indent}Execute the following actions sequentially:\n"
            else:
                description += f"{indent}Execute the following actions:\n"
...
# Extract action IDs and count from the main tree
main_tree = root.find(".//BehaviorTree[@ID='MainTree']")
action_ids, action_count = extract_action_ids_and_count(main_tree)

print("\nTotal number of actions in the main tree:", action_count)

# Generate and print natural language description of the behavior tree
tree_description = generate_tree_description(main_tree)
